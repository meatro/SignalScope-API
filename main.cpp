...
void setup() {
    ...
    Serial.printf("[runtime] mode=%s hw_cores=%d pref_dual=%d | CAN core=%d created=%d | UI core=%d created=%d\n",
...
void handleReplayControl() {
    ...
    if (bodyContains(action_text, "start")) {
        ...
        const String json = "{\"ok\":true,\"action\":\"start\",\"start_delay_us\":" + String(start_delay_us) + "}";
        server.send(200, "application/json", json);
        return;
    }
...
void handleReplaySend() {
    const uint32_t can_id = parseUIntArg("can_id", 0U);
    const uint32_t dlc_arg = parseUIntArg("dlc", 8U);
    const uint8_t dlc = static_cast<uint8_t>((dlc_arg > 8U) ? 8U : dlc_arg);
    const Direction direction = parseDirectionFromText(server.arg("direction"), Direction::A_TO_B);
    ...
    if (!replay_engine.loadLogCsv(csv.c_str(), static_cast<size_t>(csv.length()), direction)) {
    ...
    const String json = "{\"ok\":true,\"frames\":" + String(static_cast<uint32_t>(replay_engine.frameCount())) +
